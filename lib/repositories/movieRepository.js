...
/**
 * Check if exist movie with specific id and movie is disponible.
 *
 * @param {number} id
 * @returns {Promise}
 */
const checkIfMovieExists = id => new Promise((resolve, reject) => {
  const queryToGetCountOfMovies = mysql('movies')
    .select('id')
    .where('id', id)
    .limit(1);

  queryToGetCountOfMovies.then((rows) => {
    if (rows.length > 0) {
      resolve();
    } else {
      reject();
